...
class preloadGame extends Phaser.Scene {
  ...
  preload() {
    this.load.image(`platform`, `../public/assets/img/platform.png`);
    // this.load.image(`city`, `./img/cityscape.jpg`);

    // player is a sprite sheet made by 24x48 pixels
    this.load.spritesheet(`player`, `../public/assets/img/player.png`, {
      frameWidth: 24,
      frameHeight: 48
    });

    // the pill is a sprite sheet made by 20x20 pixels
    this.load.spritesheet(`pill`, `../public/assets/img/coin.png`, {
      frameWidth: 20,
      frameHeight: 20
    });

    // the bacteriacamp is a sprite sheet made by 32x58 pixels
    this.load.spritesheet(`bacteria`, `../public/assets/img/bacteria.png`, {
      frameWidth: 40,
      frameHeight: 70
    });

    // buildings are a sprite sheet made by 512x512 pixels
    this.load.spritesheet(`building`, `../public/assets/img/buildings.png`, {
      frameWidth: 512,
      frameHeight: 512
    });
  }
  create() {
    // this.add.image(850, 700, `city`);
...
class playGame extends Phaser.Scene{
    ...
    create(){
        ...
        this.physics.add.overlap(this.player, this.pillGroup, function(player, pill){
        ...
        this.physics.add.overlap(this.player, this.bacteriaGroup, function(player, bacteria){
    ...
    addBuildings(){
        ...
        if(rightmostBuilding < game.config.width * 2){
            let building = this.physics.add.sprite(rightmostBuilding + Phaser.Math.Between(100, 350), game.config.height + Phaser.Math.Between(0, 100), `building`);
        ...
        if(this.addedPlatforms > 1){
            ...
            if(Phaser.Math.Between(1, 100) <= gameOptions.bacteriaPercent){
                if(this.bacteriaPool.getLength()){
                    ...
                    bacteria.x = posX - platformWidth / 2 + Phaser.Math.Between(1, platformWidth);
                    bacteria.y = posY - 46;
                    bacteria.alpha = 1;
                    ...
                    let bacteria = this.physics.add.sprite(posX - platformWidth / 2 + Phaser.Math.Between(1, platformWidth), posY - 46, `bacteria`);
